...
function is31Flavors(arr){
    if(arr.length == 31){
        console.log("it is 31 flavors");
    }
    else{
        console.log("It is not 31 flavors");
    }
...
let newArr = [];
function copy(newA, original){
    newA = [...original];
    console.log(newA);
}
copy(newArr,otherCopy);
...
function filterByWord(arr){
    let z = 0;
    for (i in arr){
        if(/\bchocolate/ig.test(arr[i])){ //insert word here
            newFlavorArr[z] = arr[i];
            z++;
        }
    }
    console.log(newFlavorArr);
}
